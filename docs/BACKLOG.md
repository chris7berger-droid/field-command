...
Last updated: 2026-09-15 (**REL1:** EAS/TestFlight release-readiness config linked. No build/submit yet.)
...
| ID  | Tier | Status | Item | Source | Notes |
|-----|------|--------|------|--------|-------|
| TF1 | T2   | Open   | First EAS iOS production build + TestFlight internal submit | REL1 2026-09-15 | **Blocked by:** paid Apple Developer Program (not free Personal Team `7T26H9PCGN`). Expo project is already linked (`@chris7berger/field-command`, projectId `9b762fbe-c0d1-46a6-a2bb-efe014e7b489`). After Chris confirms paid enrollment: `npx eas-cli build --platform ios --profile production`, review the binary, then submit. Expo Go is not valid. Do not redesign env into EAS-managed environments in the same breath. |
...
| REL1 | T1   | 2026-09-15 | EAS / TestFlight iOS release-readiness configuration | **CONFIG READY, no binary yet.** Branch `feat/eas-testflight-ios`. Expo `@chris7berger/field-command`, projectId `9b762fbe-c0d1-46a6-a2bb-efe014e7b489`. Production profile is store/TestFlight. Explicit `EXPO_PUBLIC_*` in `eas.json` + `.env`; runtime fail-closed; no service-role. First build is **TF1**, gated on paid Apple Developer Program. Local Xcode team `7T26H9PCGN` is not the TestFlight path. |
