...
Last updated: 2026-09-15 (**AG1 verified:** fail-close Field Command activation gate on native iOS. Not merged until Chris approves.)
...
|-----|------|------------|------|------------|
| AG1 | T1   | 2026-09-15 | Fail-close Field Command activation + canonical writer identity | **VERIFIED on native iOS, pending merge.** Branch `cursor/production-activation-gate-a21d`, implementation `bb0a7b6`. Login requires an active `team_members` row whose `apps` includes `"field"`. Writes use `team_members.id` only. Chris runtime-verified on iPhone 17 simulator via `npx expo run:ios`: entitled account reached Home with live job/crew data; non-entitled account stopped on FIELD COMMAND NOT ACTIVE; SIGN OUT returned to login. **Expo Go is not a valid runtime** (PowerSync / OP-SQLite). No PowerSync reset required. Do not merge until Chris approves. |
