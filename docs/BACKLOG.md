...
Last updated: 2026-09-13 (**FE3 filed:** office punch-time correction after a missed clock-out — ask the crew the real leave time and match other crew on that job. Tied to office time-clock work, not this session. Accidental close on the phone is punch-out-now + notify office. Night work stays the same shift past midnight.)
...
| FE2 | T2   | In Progress | Sync Schedule trip titles into Field SOW (replace `WTC N`) | Field launch smoke 2026-09-11 | Client schema + sync-rules draft add `job_mobilizations`. Titles will not reach phones until the Postgres `powersync` publication + dashboard rules include the table. Fallback label is `Trip N`. Test job 10176 / 1284: TAP + Sing. |
